	...
	QUIPS =
	{
		quip_scout_generic =
		{
			...
			[[
				...
				!closeeyes
				I look forward to getting back in the air.
			]],
			[[
				!gesture
		...
		quip_scout_no_dojo = {
			[[
				...
				!gesture
				Maybe he'll be here by the time we get back from our next {name.run}.
			]],
			[[
				!shrug
			...
			[[
				...
				!nervous
				He can handle himself, but it's a little crazy out there.
			]],
			[[
				!thinking
		...
		quip_won_last_run = {
			[[
				!think
				I didn't think to pack bug spray for this expedition.
				!dubious
				I regret that.
			]],
			[[
			...
			[[
				...
				!think
				I have to brush it kinda thoroughly afterward to prevent matting, though.
			]],
			[[
				!laugh
	...
	{	
		dgn_tips_scout = {
			POLEARM_FULL_CONVO = {
				...
				OPT_1A = "I wouldn't mind some tips.",
				OPT_1B = "Yep! I'm good.",
				...
				OPT_2A = "How do I Rolling Drill?",
				OPT_END = "Thanks, {name.npc_scout}!",

				OPT2A_RESPONSE = --> go to OPT_END
				[[
					agent:
						!point
		...
		main_defeat_owlitzer =
		{
			...
			quest_intro =
			{
				...
				OPT_1D = "Do you have any leads on our missing crew?",
			...
			celebrate_defeat_boss =
			{
				...
				TALK_FIRST_BOSS_KILL = [[
					agent:
						!shocked
						I can't believe you've taken down <i>two</i> unique <#RED>{name_multiple.rot_boss}</> since we met!
						!think
						...But now the real fun begins.
						!gesture
						Each new <#RED>Frenzy Level</> you beat will yield a <#PURPLE>{name.i_konjur_heart}</>...
						!shocked
						And you just unlocked a whole new gamut of <#RED>Frenzy Levels</> by defeating that <#RED>{name.owlitzer}</>!
						!dubious
