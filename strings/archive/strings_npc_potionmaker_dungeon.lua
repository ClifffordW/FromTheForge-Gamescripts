	...
	QUESTS =
	{
		first_meeting = {
			...
			OPT4A_RESPONSE = [[
				agent:
					!clap
					I knew you were a smart <#BLUE>{player.species}</>.
					!greet
			...
			player_emptied_flask = {
				...
				RESPONSE_OPT1C = [[
					agent:
						...
						Now hold on just a sssecond there!
						!dubious
						I'll let you in on a little secret, my good <#BLUE>{player.species}</>. These woods are <i>daaanger</i>-ous. 
						!think
			...
			buffer = {
				...
				TALK = [[
					agent:
						!dubious
						It's been a pleasssure to meet a smart and savvy <#BLUE>{player.species}</> such as yourself!
						!greet
						Best of luck to you.
				]],
			},
		...
		{
			live_here = {
				QUESTION = "I was wondering... do you live out here?",
				ANSWER = [[
					agent:
						!shocked
						HEAVENSSS, no.
						!point
						I don't associate with the rabble that scampers these woods.
						!closedeyes
						I went to <i>busssiness</i> school.
				]],
			},

			what_brought_you = {
				QUESTION = "So what brought you out here, {name.npc_doc_lastname}?",
				ANSWER = [[
					agent:
						!gesture
						I go where my wares are needed.
						!agree
						And you, my <#KONJUR>{name.konjur}</>-splattered friend, are in dire need!
				]],
			},
		},
		...
		dgn_business_ventures_potion = 
		{
			...
			{	--TODO kris remove agent ...
				TALK = [[
					agent:
						...
					player:
						{name.npc_doc_lastname}? How'd you end up in <#BLUE>{name.bandi_swamp}</>?
					agent:
						!angry
						You know what they say, my good <#BLUE>{player.species}</>-- location, location, location!!
						!dubious
				...
				OPT_3C = "Eh, why bother.",
					...
					--BRANCH TWO--
						OPT3A_RESPONSE = [[
							...
							agent:
								!gesture
								Here'sss your bag back, my good <#BLUE>{player.species}</>.
						]],
			...
			sedament_tundra = {
				...
				OPT1B_RESPONSE_ALT_NOFUNDS = [[
					!shocked
						Why, my dear <#BLUE>{player.species}</>, it appears you haven't the <#KONJUR>{name.i_konjur}</> for this ssspecialty item!
						!dejected
						What a ssshame.
						!greet
				...
				OPT1B_RESPONSE_ALT_NOSPACE = [[
					agent:
						!shocked
						Why, my dear <#BLUE>{player.species}</>, you have no room for this specialty <#RED>Potion</>!
						!shrug
		...
		dgn_shop_potion =
		{
			...
			shop =
			{
				TALK_MINIGAME = [[
					agent:
						!dubious
						Why, my good <#BLUE>{player.species}</>! Your flask's run bone dry!
						!gesture
